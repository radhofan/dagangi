...
export default function DebtDetailScreen() {
  ...
  async function pay() {
    try {
      await recordDebtPayment(id, parseRupiahInput(amount));
      setAmount('');
      await debt.refresh();
    ...
    <Screen>
      ...
      {remaining > 0 ? (
        <AppCard title="Catat Pembayaran">
          <FormField label="Jumlah Bayar" value={amount} onChangeText={(value) => setAmount(formatRupiahInput(value))} keyboardType="numeric" />
